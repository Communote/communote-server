package com.communote.plugins.api.rest.resource.timelinenote;
...
import com.communote.plugins.api.rest.resource.note.NoteResourceHelper;
...
GetTimelineNoteParameter, GetCollectionTimelineNoteParameter> {
    ...
    private final static Logger LOG = Logger.getLogger(TimelineNoteResourceHandler.class);
     ...
     */
    private NoteQueryParameters configureQueryInstance(Map<String, ?> parameters,
            Request request) {
    ...
    private PageableList<TimelineNoteResource> filterNotes(NoteQueryParameters noteQueryParameters,
        ...
        if (noteQueryParameters.isThreadedView()) {
            ...
            timelineNoteListItems = ServiceLocator.findService(QueryManagement.class)
     ...
     */
    private Map<String, ?> generateParameterMap(
    ...
    public Response handleGetInternally(GetTimelineNoteParameter getTimelineNoteParameter,
        ...
        NoteQueryParameters noteQueryInstance = configureQueryInstance(
        ...
        List<TimelineNoteResource> resultTimelineNotes = filterNotes(noteQueryInstance, filterHtml,
                request);
        if (resultTimelineNotes.size() != 1) {
            LOG.debug("More Notes with Identifier " + getTimelineNoteParameter.getNoteId()
                    + " are available.");
