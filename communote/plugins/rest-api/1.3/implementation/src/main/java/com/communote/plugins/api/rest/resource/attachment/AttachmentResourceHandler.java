...
public class AttachmentResourceHandler
        extends
        DefaultResourceHandler<CreateAttachmentParameter, DefaultParameter, DefaultParameter, GetAttachmentParameter, DefaultParameter> {
     ...
     */
    private Response getAttachmentAsBinary(AttachmentTO attachment) throws IOException {
    ...
    protected Response handleCreateInternally(CreateAttachmentParameter createParameter,
            ...
            throws ParameterValidationException, ResponseBuildException, Exception {

        HttpServletRequest httpReq = RequestHelper.getHttpServletRequest(org.restlet.Request
                .getCurrent());

        // get parameter directly from request because createParameter is null for this mime type
        AttachmentResourceHelper.checkAttachmentUploadSessionId(httpReq
                .getParameter("attachmentUploadSessionId"));

        return ResponseHelper.buildSuccessResponse(extractAttachments(httpReq, request), request);
    }
    ...
    public Response handleGetInternally(GetAttachmentParameter getAttachmentParameter,
        ...
        attachment = AttachmentResourceHelper.getResourceStoringManagement().getAttachment(
                attachmentId);

        Boolean asBinary = getAttachmentParameter.getGetAttachmentAsBinary();
        if (asBinary != null && asBinary) {
            return getAttachmentAsBinary(attachment);
        } else {
