package com.communote.plugins.api.rest.v30.to;
...
public class ApiResult<T> implements Serializable {
    ...
    static {
        if (CommunoteRuntime.getInstance().getConfigurationManager().getDevelopmentProperties()
                .isDebugRestApi()) {
            OBJECT_WRITER = new ObjectMapper().writerWithView(DevelopmentView.class);
        } else {
            OBJECT_WRITER = new ObjectMapper().writerWithView(PublicView.class);
        }
    }
    /**
     *
    ...
    private List<ApiResultError> errors;
