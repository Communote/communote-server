...
import com.communote.plugins.api.rest.v30.converter.ExternalObjectListItemToExternalObjectResourceConverter;
import com.communote.plugins.api.rest.v30.converter.ExternalObjectToExternalObjectResourceConverter;
...
public class ExternalObjectResourceHandler
    ...
    public Response handleListInternally(
        ...
        final List<ExternalObjectResource> externalObjectResources = new ArrayList<ExternalObjectResource>();
        Long topicId = TopicResourceHelper
                .getTopicIdByIdentifier(
        ...
        if (topicId.longValue() == 0) {
            ...
            if (getCollectionExternalObjectParameter.getExternalId() == null
                    || getCollectionExternalObjectParameter.getExternalSystemId() == null) {
                throw new IllegalRequestParameterException("externalId || externalSystemId", null,
                        "Both parameters must be set when filtering over all topics.");
            }
            ExternalObjectQuery query = QueryDefinitionRepository.instance().getQueryDefinition(
                    ExternalObjectQuery.class);
            ExternalObjectQueryParameters params = query.createInstance();

            params.setResultSpecification(new ResultSpecification(0, 10));
            params.setExternalId(getCollectionExternalObjectParameter.getExternalId());
            params.setExternalSystemId(getCollectionExternalObjectParameter.getExternalSystemId());

            ExternalObjectListItemToExternalObjectResourceConverter<ExternalObjectListItem, ExternalObjectResource> converter =
                    new ExternalObjectListItemToExternalObjectResourceConverter<>();
            List<ExternalObjectListItem> externalObjects = ServiceLocator.findService(
                    QueryManagement.class).query(query, params);
            for (ExternalObjectListItem externalObjectListItem : externalObjects) {
                ExternalObjectResource externalObjectResource = new ExternalObjectResource();
                converter.convert(externalObjectListItem, externalObjectResource);
            ...
            ExternalObjectToExternalObjectResourceConverter<ExternalObject, ExternalObjectResource> externalObjectToExternalObjectResourceConverter =
                    new ExternalObjectToExternalObjectResourceConverter<ExternalObject, ExternalObjectResource>();

            for (ExternalObject externalObject : externalObjects) {
                ExternalObjectResource externalObjectResource = new ExternalObjectResource();
                externalObjectToExternalObjectResourceConverter
                        .convert(externalObject, externalObjectResource);
                externalObjectResources.add(externalObjectResource);
            }
        }
