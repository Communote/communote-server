...
public final class AttachmentResourceHelper {
    ...
    public static void assertAttachmentSize(String contentType, Long fileSize, boolean isBase64)
            throws MaxLengthReachedException, AttachmentIsEmptyException {
        long maxAttachmentSize = 0;
        if (fileSize <= 0) {
            throw new AttachmentIsEmptyException();
        }
        ApplicationConfigurationProperties appProps = CommunoteRuntime.getInstance()
                .getConfigurationManager().getApplicationConfigurationProperties();
        if (contentType.startsWith(CONTENT_TYPE_IMAGE_PREFIX)) {
            maxAttachmentSize = Long.parseLong(appProps
    ...
    public static AttachmentResource buildAttachmentResource(AttachmentData attachment) {
        ...
        attachmentResource.setContentId(attachment.getContentId());
        attachmentResource.setRepositoryConnectorId(attachment.getRepositoryId());
        attachmentResource.setNoteId(attachment.getNoteId());
    ...
    public static AttachmentResource buildAttachmentResource(AttachmentTO attachment) {
        ...
        if (attachment.getMetadata() != null && attachment.getMetadata().getContentId() != null) {
            attachmentResource.setContentId(attachment.getMetadata().getContentId().getContentId());
            attachmentResource.setRepositoryConnectorId(attachment.getMetadata().getContentId()
                    .getConnectorId());
        }
        attachmentResource.setNoteId(attachment.getNoteId());
        return attachmentResource;
