...
import com.communote.plugins.api.rest.v22.resource.validation.DefaultValidator;
 ...
 */
class NoteResourceValidator
        extends
DefaultValidator<CreateNoteParameter, EditNoteParameter, DeleteNoteParameter, GetNoteParameter, DefaultParameter> {
    ...
    public void validateEdit(EditNoteParameter editPrameter) throws ParameterValidationException {
        ...
        errors.addAll(checkTopicIdExistence(editPrameter.getTopicId()));
        if (errors.size() > 0) {
            ParameterValidationException exception = new ParameterValidationException();
            exception.setErrors(errors);
            throw exception;
