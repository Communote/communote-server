...
public class InstallerAdminAccountValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        // check user password
        ValidationHelper.validatePasswords(ServiceLocator.findService(UserPasswordManagement.class),
                "userPassword", form.getUserPassword(), "userPasswordConfirmation",
                form.getUserPasswordConfirmation(), errors);
    }
    ...
    private void validateAlias(String fieldName, String input, Errors errors) {
        ...
        if (!errors.hasFieldErrors(fieldName) && !input.matches(ValidationPatterns.PATTERN_ALIAS)) {
