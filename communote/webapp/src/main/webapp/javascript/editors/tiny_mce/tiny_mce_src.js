...
(function(win) {
	...
	var tinymce = {
		...
		_init : function() {
			...
			// ie 11 support
			if (!t.isIE &&  ua.indexOf('Trident/') != -1 && (ua.indexOf('rv:') != -1 || na.appName.indexOf('Netscape') != -1)) {
			    t.isIE = true;
			    t.isIE11 = true;
			}
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getRng : function(w3c) {
            ...
            // createRange can cause an "access denied" error if kbd focus is for instance outside tinymce iframe inside
            // a file input (probably IE bug since doc is the iframe document which shouldn't care about a selection outside
            // the iframe). workaround: check whether it is actually a control selection
			// IE11 does not support document.selection anymore, check this too
            if (tinymce.isIE && rng && rng.setStart && doc.selection && doc.selection.type === 'Control' && doc.selection.createRange().item) {
			...
			dom.bind(doc, ['mousedown', 'contextmenu'], function(e) {
				...
				if (e.target.nodeName === 'HTML') {
					...
					if (htmlElm.scrollHeight > htmlElm.clientHeight) {
                        // will lose focus when scrollbars were clicked
                        // using a timeout here since ever 2nd click or so won't refocus the editor otherwise
                        setTimeout(function() {
                            editor.getWin().focus();
                        }, 0);
						return;
					...
					rng = rngFromPoint(e.x, e.y);
					if (rng) {
					    // selection should only start when clicking the left mouse button
                        if (e.type === 'mousedown' && (e.which === 1 || e.button === 1)) {
                            started = 1;
                            startRng = rng;
					    ...
					    // if the body of the iframe for instance has a padding the click will not have a range
					    if (editor !== tinymce.activeEditor) {
					        editor.focus();
					        return false;
					    } else {
					        // do nothing if the caret is in the iframe, otherwise focus
					        if (!document.activeElement || editor.id + '_ifr' !== document.activeElement.id) {
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.MenuButton:tinymce.ui.Button', {
		...
		showMenu : function() {
			var t = this, p1, p2, e = DOM.get(t.id), m, co;
            ...
            co = DOM.get('menu_' + m.id);
            DOM.setStyles(co, {left: p2.x - co.offsetWidth + e.clientWidth});
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		setContent : function(content, args) {
			...
			if ((!tinymce.isIE || tinymce.isIE11) && (content.length === 0 || /^\s+$/.test(content))) {
	...
	tinymce.EditorCommands = function(editor) {
		...
		addCommands({
			...
			'Indent,Outdent' : function(command) {
				...
				// respect inline styles settings
			...
			function removeCaretContainer(node, move_caret) {
				...
				if (!node) {
					...
					if (isCaretContainerEmpty(node)) {
						...
						// backup range details since the remove can change the range in that the parent container will become the new start/end
						rngDetails = {
						        ...
						        eo: rng.endOffset};
						dom.remove(node, 1);
						if (rng.startContainer != rngDetails.s) {
						    rng.setStart(rngDetails.s, rngDetails.so);
						    rng.setEnd(rngDetails.e, rngDetails.eo);
						}
					}
		...
		function handleEnterKey(evt) {
			...
			function insertBr() {
				...
				if (container && container.nodeType == 3 && offset >= container.nodeValue.length) {
					...
					if ((!tinymce.isIE || tinymce.isIE11) && !hasRightSideBr()) {
						brElm = dom.create('br');
