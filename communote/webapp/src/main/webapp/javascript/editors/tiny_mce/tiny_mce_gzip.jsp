 ...
 *
 * RWI: +removed linebreak after first line to avoid IllegalStateException: getOutputStream() has already been called
 *      +mapPath modified to build correct path (original was like: ...\\webappname\\webappname\\...)
 *		+changed usage of implicit out object into usage of outStream to avoid IllegalStateException: getOutputStream() has already been called
 */

    // AMO: Set not modifed, when "If-Modified-Since" header is present. 
	...
	*/};
    ...
    response.addHeader("Cache-Control", "max-age=" + expiresOffset + ", public");
	response.setDateHeader("Expires", System.currentTimeMillis() + (expiresOffset * 1000));
    response.setDateHeader("Last-Modified",CommunoteRuntime.getInstance().getApplicationInformation().getBuildTimestamp());

	// Is called directly then auto init with default settings
	if (!isJS) {
		outStream.print(getFileContents(mapPath(request, "tiny_mce_gzip.js")));
		outStream.print("tinyMCE_GZ.init({});");
		outStream.close();
		return;
	}
	...
	if (supportsGzip) {
		...
		outStream.write(content.getBytes("iso-8859-1"));
		outStream.close();
%><%!
	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	public String mapPath(HttpServletRequest request, String path) {
		...
		if (absPath.endsWith("/") && reqPath.startsWith("/"))
			reqPath = reqPath.substring(1).replace('/', File.separatorChar);
		else
