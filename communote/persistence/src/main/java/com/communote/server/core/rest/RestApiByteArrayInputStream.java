 ...
 * @since JDK1.0
 */
public class RestApiByteArrayInputStream extends ServletInputStream {

    /**
     * An array of bytes that was provided by the creator of the stream. Elements
     * <code>buf[0]</code> through <code>buf[count-1]</code> are the only bytes that can ever be
     * read from the stream; element <code>buf[pos]</code> is the next byte to be read.
     */
     ...
     * be read from the input stream buffer will be <code>buf[pos]</code>.
     */
    private int pos;
     ...
     *            the input buffer.
     */
    public RestApiByteArrayInputStream(byte[] buf) {
        this.buf = buf;
        this.pos = 0;
     ...
     */
    public RestApiByteArrayInputStream(byte[] buf, int offset, int length) {
        this.buf = buf;
        this.pos = offset;
     ...
     * stream.
     * <p>
     * The value returned is <code>count&nbsp;- pos</code>, which is the number of bytes remaining
     ...
     * <p>
     * If no mark has been set, then the value of the mark is the offset passed to the constructor
     ...
     *            The <code>readAheadLimit</code> for this class has no meaning.
     */
    @Override
    ...
    /**
     * Resets the buffer to the marked position. The marked position is 0 unless another position
     * was marked or an offset was specified in the constructor.
     */
    @Override
