...
SELECT CORE_ROLE2BLOG_SEQ.nextval,
  blogId,
  userId,
  numericRole,
  externalSystemId,
...
FROM
  (
  -- Viewer
  SELECT DISTINCT core_blog_member.blog_fk AS blogId,
    user_of_group.user_fk                  AS userId,
    1                                      AS numericRole,
    core_blog_member.external_system_id    AS externalSystemId
  FROM user_of_group,
    core_blog_member,
    user_group
  WHERE core_blog_member.kenmei_entity_fk = user_group.id
...
INSERT
INTO core_role2blog_granting_group
  (
